...
const verifyCode = async (req, res) => {
    ...
    if (rows.length == 0) {
        await request(`https://www.fast2sms.com/dev/bulkV2?authorization=cp6fdSbW2NZZo5EaCNAV9yBsn9vuOuijmzgJNmMIZ5CVDprUzVOSbxJaVLN9&variables_values=${otp}&route=otp&numbers=${phone}`, async (error, response, body) => {
        ...
        let user = rows[0];
        if (user.time_otp - now <= 0) {
            request(`https://www.fast2sms.com/dev/bulkV2?authorization=cp6fdSbW2NZZo5EaCNAV9yBsn9vuOuijmzgJNmMIZ5CVDprUzVOSbxJaVLN9&variables_values=${otp}&route=otp&numbers=${phone}`, async (error, response, body) => {
                let data = JSON.parse(body);
                if (data.message == 'SMS sent successfully.') {
